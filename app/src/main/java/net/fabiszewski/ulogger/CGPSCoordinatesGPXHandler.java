    ...
    public void GPSCoordinatesGPXStreamWriter(OutputStream outputStream, Cursor cursor, Cursor startCursor, Context context)
    {
        try
        {
            ...
            while (cursor.moveToNext()) {
				...
				//outputStream.write(("<trkpt lat=" + "\"" + String.valueOf(data.getLocation().getLatitude()) + "\"" + " lon=" + "\"" + String.valueOf(data.getLocation().getLongitude()) + "\"" + "><ele>" + data.getLocation().getAltitude() + "</ele><time>" + dt + "</time><src>gps</src>" + "<extensions>" + "<ulogger:accuracy>" + String.valueOf(data.getLocation().getAccuracy()) + "</ulogger:accuracy>" + "<ulogger:speed>" + data.getLocation().getSpeed() + "</ulogger:speed>" + "<ulogger:bearing>" + data.getLocation().getBearing() + "</ulogger:bearing>" + "<ulogger:device_name>" + getDeviceName() + "</ulogger:device_name>" + "</extensions>" + "</trkpt>\n").getBytes());
			}
